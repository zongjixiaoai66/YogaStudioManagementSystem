    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="yonghuId" name="yonghuId" type="hidden">
               <el-col :span="12">
                   <el-form-item class="input" v-if="type!='info'"  label="报告标题" prop="ticebaogaoName">
                       <el-input v-model="ruleForm.ticebaogaoName"
                                 placeholder="报告标题" clearable  :readonly="ro.ticebaogaoName"></el-input>
                   </el-form-item>
                   <div v-else-if="type=='info'">
                       <el-form-item class="input" label="报告标题" prop="ticebaogaoName">
                           <el-input v-model="ruleForm.ticebaogaoName"
                                     placeholder="报告标题" readonly></el-input>
                       </el-form-item>
                   </div>
               </el-col>
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="报告类型" prop="ticebaogaoTypes">
                        <el-select v-model="ruleForm.ticebaogaoTypes" :disabled="ro.ticebaogaoTypes" placeholder="请选择报告类型">
                            <el-option
                                v-for="(item,index) in ticebaogaoTypesOptions"
                    ...
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="报告类型" prop="ticebaogaoValue">
                        <el-input v-model="ruleForm.ticebaogaoValue"
                            placeholder="报告类型" readonly></el-input>
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="24">
                    <el-form-item v-if="type!='info'"  label="报告详情" prop="ticebaogaoContent">
                        <editor style="min-width: 200px; max-width: 600px;"
                                v-model="ruleForm.ticebaogaoContent"
                                class="editor"
                                action="file/upload">
                        </editor>
                    </el-form-item>
                    <div v-else-if="type=='info'">
                        <el-form-item v-if="ruleForm.ticebaogaoContent" label="报告详情" prop="ticebaogaoContent">
                            <span v-html="ruleForm.ticebaogaoContent"></span>
                        </el-form-item>
                    </div>
                </el-col>
            </el-row>
    ...
    export default {
        data() {
            return {
                ...
                ro:{
                    yonghuId: false,
                    ticebaogaoName: false,
                    ticebaogaoTypes: false,
                    insertTime: false,
                    ticebaogaoContent: false,
                },
                ruleForm: {
                    yonghuId: '',
                    ticebaogaoName: '',
                ...
                rules: {
                   ...
                   ticebaogaoContent: [
                              { required: true, message: '报告详情不能为空', trigger: 'blur' },
                          ],
                }
            };
        },
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=ticebaogao_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.ticebaogaoTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            info(id) {
                let _this =this;
                _this.$http({
                    url: `ticebaogao/info/${id}`,
                    method: 'get'
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        _this.ruleForm = data.data;
                        _this.yonghuChange(data.data.yonghuId)
                        _this.ruleForm.ticebaogaoContent = _this.ruleForm.ticebaogaoContent.replaceAll("src=\"upload/","src=\""+this.$base.url+"upload/");
                    } else {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.ruleForm.ticebaogaoContent = this.ruleForm.ticebaogaoContent.replaceAll(this.$base.url,"");
                        this.$http({
                            url:`ticebaogao/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.ticebaogaoCrossAddOrUpdateFlag = false;
                                        this.parent.search();
